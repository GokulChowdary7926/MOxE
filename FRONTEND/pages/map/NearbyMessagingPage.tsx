...
export default function NearbyMessagingPage() {
  ...
  useEffect(() => {
    ...
    if (!token) return;
    fetch(`${getApiBase()}/location/nearby-post/usage`, { headers: { Authorization: `Bearer ${token}` } })
  ...
  useEffect(() => {
    if (user?.id) initializeSocket(user.id, accountIdFromStore ?? currentAccount?.id);
    let cancelled = false;
      ...
      socket.on('nearby:message', (payload: any) => {
        ...
        setMessages((prev) => {
          ...
          if (isOwnEcho) {
            ...
            if (idx >= 0) {
              const next = [...kept];
              next[idx] = { ...next[idx], fromUsername: fromUsername ?? undefined, fromDisplayName: fromDisplayName ?? undefined, fromUserId };
  ...
  }, [user?.id, accountIdFromStore, currentAccount?.id]);
  ...
  const handleSend = () => {
    ...
    setMessages((prev) => {
      const now = Date.now();
      const kept = prev.filter((m) => now - new Date(m.sentAt).getTime() < MESSAGE_RETENTION_MS);
      return [{ text, fromUserId: null, fromDisplayName, fromUsername, sentAt, replyCount: 0, likeCount: 0 }, ...kept].slice(0, 200);
    });
    setMessage('');
    ...
    if (pendingPost) {
      ...
      try {
        ...
        setMessages((prev) => {
          const now = Date.now();
          const kept = prev.filter((m) => now - new Date(m.sentAt).getTime() < MESSAGE_RETENTION_MS);
          return [{ text: caption, fromUserId: null, fromDisplayName, fromUsername, sentAt, imageUrl, replyCount: 0, likeCount: 0 }, ...kept].slice(0, 200);
        });
  ...
  const isOwnMessage = (m: NearbyMessage) =>
    m.fromUserId === null || (currentAccount != null && m.fromUsername === currentAccount.username && m.fromUsername !== 'anonymous');
  ...
  const messageKey = (m: NearbyMessage, idx: number) => `${m.sentAt}-${idx}-${(m.text + (m.imageUrl || '')).slice(0, 40)}`;
  const visibleMessages = messages.filter((m) => Date.now() - new Date(m.sentAt).getTime() < MESSAGE_RETENTION_MS);
                    ...
                    <div className="flex gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                            ...
                            {menuOpen && (
                              <>
                                ...
                                <div className="absolute right-0 top-full mt-1 py-1 rounded-xl bg-[#16181c] border border-[#2f2f2f] shadow-xl z-20 min-w-[160px]">
                                  <button type="button" onClick={() => { navigator.clipboard?.writeText(m.text || ''); setOpenMenuKey(null); }} className="w-full px-4 py-2.5 text-left text-white text-sm hover:bg-[#2f2f2f]">Copy text</button>
                                  <button type="button" onClick={() => setOpenMenuKey(null)} className="w-full px-4 py-2.5 text-left text-white text-sm hover:bg-[#2f2f2f]">Report</button>
                                </div>
                              </>
                            )}
                          </div>
                        ...
                        <div className="flex items-center justify-between mt-2 max-w-[320px] text-[#71767b]">
                          <button type="button" className="flex items-center gap-1.5 p-1 -m-1 hover:text-[#1d9bf0]" onClick={() => setOpenMenuKey(null)} aria-label="Comment">
                            <MessageSquare className="w-4 h-4" />
                            <span className="text-[13px]">{m.replyCount ?? 0}</span>
                          </button>
                          <button type="button" className="flex items-center gap-1.5 p-1 -m-1 hover:text-[#1d9bf0]" aria-label="Repost">
                          ...
                          <button type="button" className="p-1 -m-1 hover:text-[#1d9bf0]" aria-label="Analytics">
                            <BarChart3 className="w-4 h-4" />
                          </button>
                          <button type="button" className="p-1 -m-1 hover:text-[#1d9bf0]" aria-label="Bookmark">
